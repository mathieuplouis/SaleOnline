...
import mum.edu.config.GMailAuthenticator;
...
//        ModelAndView modelAndView = new ModelAndView();
    ...
    public String adminForm(@ModelAttribute("admin") User user, Model model) {
        return "createAdmin";
    }

    @PostMapping(value = "/admin/createAdmin")
    public String addAdmin(@Valid @ModelAttribute("admin") User user, BindingResult result, Model model) {
        if (result.hasErrors()) {
    ...
    public String getAdmin(@PathVariable Long id, Model model) {
        ...
        model.addAttribute("admin", userFind);
        return "editAdmin";
    }

    @PostMapping(value = "/admin/editAdmin")
    public String editAdmin(@Valid @ModelAttribute("admin") User user, BindingResult result, Model model) {
        if (result.hasErrors()) {
            model.addAttribute("errormessage", "Enregistrement echoue!");
            model.addAttribute("admin", user);
            return "createAdmin";
    ...
    public String registrationUserForm(@ModelAttribute("userRegistration") User user, Model model) {
    ...
    public String addUser(@Valid @ModelAttribute("userRegistration") User user, BindingResult result, Model model) {
        if (result.hasErrors()) {
            model.addAttribute("errormessage", "Enregistrement echoue!");
    ...
    public String getRegistrations(@PathVariable Long id, Model model) {
        User userFind = userService.findById(id);
        model.addAttribute("userRegistration", userFind);
    ...
    public String editUser(@Valid @ModelAttribute("userRegistration") User user, BindingResult result, Model model) {
        if (result.hasErrors()) {
            model.addAttribute("errormessage", "Enregistrement echoue!");
            model.addAttribute("userRegistration", user);
            return "UserRegistrationForm";
    ...
    public String approvalUser(@Valid @ModelAttribute("user") User user, BindingResult result, RedirectAttributes attr, Model model) {
    ...
    private void sendEmail() throws Exception {
        ...
        String from = "vocsi130@gmail.com";
        String pass = "passvocsi130";
        ...
        InternetAddress toAddress = new InternetAddress("vocsi130@gmail.com");
        ...
        message.setText("Welcome to Coza Store! \n Your are approved!");
...
//        model.addAttribute("productList",productService.findAll());
    ...
    public String paymentList(Model model) {
        ...
        for (Payment adr : addressList) {
            ...
            else if (adr.getCardType() == CardType.VISA)
                adr.setCardValue("VISA");
            else if (adr.getCardType() == CardType.MASTERCARD)
                adr.setCardValue("MASTERCARD");
            else if (adr.getCardType() == CardType.AMEX)
                adr.setCardValue("AMEX");
            else if (adr.getCardType() == CardType.DISCOVER)
    ...
    public String viewFollow(@RequestParam("buyer_id") Long buyer_id, @RequestParam("seller_id") Long seller_id, @RequestParam("description") String description, @ModelAttribute("follow") Follow follow, Model model) {
